...
import { useUpdateNavProvider } from "@/utils/writeMehods";

export const UpdateNavProvider = ({ setSnackbar }) => {
  const [inputValue, setInputValue] = useState("");
  const methodName = "updateNavProvider";

  const ctx = useAppContext();
  const vaultId = ctx.selectedAsset.solanaVaultId;
  const { execute, loading: isLoading, value } = useUpdateNavProvider(vaultId);
