...
import { useRedeem } from "@/utils/anchorHelpers";

export const Redeem = ({ setSnackbar }) => {
  const [inputValue, setInputValue] = useState("");
  const methodName = "redeem";

  const ctx = useAppContext();
  const vaultId = ctx.selectedAsset.solanaVaultId;
  const { onRedeem, loading: isLoading, value } = useRedeem({ vaultId });
