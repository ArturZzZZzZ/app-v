...
import { useChangeAdmin } from "@/utils/anchorHelpers";

export const ChangeAdmin = ({ setSnackbar }) => {
  const [inputValue, setInputValue] = useState("");
  const methodName = "changeAdmin";

  const ctx = useAppContext();
  const vaultId = ctx.selectedAsset.solanaVaultId;
  const { changeAdmin, loading: isLoading, value } = useChangeAdmin(vaultId);
