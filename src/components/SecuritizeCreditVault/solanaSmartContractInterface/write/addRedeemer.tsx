...
import { useAddRedeemer } from "@/utils/anchorHelpers";

export const AddRedeemer = ({ setSnackbar }) => {
  const [inputValue, setInputValue] = useState("");
  const methodName = "addRedeemer";

  const ctx = useAppContext();
  const vaultId = ctx.selectedAsset.solanaVaultId;
  const { addRedeemer, loading: isLoading, value } = useAddRedeemer(vaultId);
