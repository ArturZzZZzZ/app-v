...
import { useLiquidate } from "@/utils/anchorHelpers";

export const Liquidate = ({ setSnackbar }) => {
  const [inputValue, setInputValue] = useState("");
  const methodName = "liquidate";

  const ctx = useAppContext();
  const vaultId = ctx.selectedAsset.solanaVaultId;
  const { onLiquidate, loading: isLoading, value } = useLiquidate({ vaultId });
