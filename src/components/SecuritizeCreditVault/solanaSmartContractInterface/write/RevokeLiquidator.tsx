...
import { useRevokeLiquidator } from "@/utils/writeMehods";

export const RevokeLiquidator = ({ setSnackbar }) => {
  const [inputValue, setInputValue] = useState("");
  const methodName = "revokeLiquidator";

  const ctx = useAppContext();
  const vaultId = ctx.selectedAsset.solanaVaultId;
  const { execute, loading: isLoading, value } = useRevokeLiquidator(vaultId);
